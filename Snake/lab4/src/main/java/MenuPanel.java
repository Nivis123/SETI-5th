...
public class MenuPanel extends JPanel {
    ...
    private final Color SUCCESS_COLOR = new Color(169, 58, 194);
    private final Color WARNING_COLOR = new Color(230, 162, 60);
    private final Color DANGER_COLOR = new Color(245, 108, 108);
    ...
    public MenuPanel(GameController ctrl) {
        this.controller = ctrl;
        setLayout(new BorderLayout(10, 10));
        setBorder(BorderFactory.createEmptyBorder(25, 25, 25, 25));
        setBackground(BACKGROUND_COLOR);

        JLabel titleLabel = new JLabel("NET SNAKE", SwingConstants.CENTER);
    ...
    private JPanel createGameCreationPanel() {
        ...
        JPanel namePanel = createLabeledField("NAME:",
                nameField = new JTextField("PLAYER" + new Random().nextInt(1000), 20));
        settingsPanel.add(namePanel, gbc);

        gbc.gridy = 1;
        gbc.gridwidth = 1;
        widthSpinner = new JSpinner(new SpinnerNumberModel(40, 10, 100, 5));
        settingsPanel.add(createLabeledField("Width:", widthSpinner), gbc);

        gbc.gridx = 1;
        heightSpinner = new JSpinner(new SpinnerNumberModel(30, 10, 100, 5));
        settingsPanel.add(createLabeledField("Hight:", heightSpinner), gbc);

        gbc.gridx = 0;
        gbc.gridy = 2;
        foodSpinner = new JSpinner(new SpinnerNumberModel(1, 0, 100, 1));
        settingsPanel.add(createLabeledField("Start food:", foodSpinner), gbc);

        gbc.gridx = 1;
        delaySpinner = new JSpinner(new SpinnerNumberModel(300, 100, 3000, 50));
        settingsPanel.add(createLabeledField("Delay (ms):", delaySpinner), gbc);
    ...
    private JPanel createGameListPanel() {
        ...
        JLabel title = new JLabel("AVAILABLE GAMES");
        title.setFont(HEADER_FONT);
        title.setForeground(TEXT_COLOR);
        title.setHorizontalAlignment(SwingConstants.CENTER);
        ...
        gamesListModel.addElement("FIND PLAY...");
        gamesListModel.addElement("(If there aren't game,create new)");
        ...
        JButton viewBtn = new JButton("OBSERVE");
    ...
    private void joinSelectedGame(boolean viewerMode) {
        ...
        if (game != null) {
            if (!game.canJoin() && !viewerMode) {
                JOptionPane.showMessageDialog(this,
                        "PLAY IS FULL! YOU CAN ONLY OBSERVE.",
                        "WARNING",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
    ...
    public void updateGamesList(List<GameInfo> games) {
        ...
        if (games.isEmpty()) {
            gamesListModel.addElement("THERE ARE NO GAMES!");
            gamesListModel.addElement("(IF THERE ARE NO GAMES,CREATE NEW GAME)");
        } else {
